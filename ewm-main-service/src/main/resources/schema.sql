...
create table if not exists comments (
    ...
    constraint uq_comment_user_id_event_id unique (user_id, event_id)
);

/* Реакции на комментарий */
create table if not exists commentreactions (
    commentreaction_id bigint generated by default as identity,
    comment_id bigint not null,
    user_id bigint not null,
    commentreaction_type varchar(10),
    constraint pk_commentreaction_id primary key (commentreaction_id),
    constraint fk_commentreaction_comment foreign key (comment_id) references comments (comment_id) on delete cascade,
    constraint fk_commentreaction_user foreign key (user_id) references users (user_id) on delete cascade,
    constraint uq_commentreaction_comment_id_user_id unique (comment_id, user_id)
);
